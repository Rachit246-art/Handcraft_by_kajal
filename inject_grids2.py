...
try:
    ...
    mg_start = col_html.find('<div class="masonry-grid" id="main-masonry-grid">')
    if mg_start == -1:
        mg_start = col_html.find('<div class="masonry-grid"')
    
    mg_end = col_html.find('</div>\n        <div class="mt-16 text-center">', mg_start)
    if mg_end == -1:
        mg_end = col_html.rfind('</div>', mg_start, col_html.find('</section>', mg_start))

    if mg_start != -1 and mg_end != -1:
        new_col_html = col_html[:mg_start+col_html[mg_start:].find('>')+2] + col_content + col_html[mg_end:]
